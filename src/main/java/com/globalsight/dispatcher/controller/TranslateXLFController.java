...
public class TranslateXLFController implements AppConstants
{
            ...
            FileUploadException, JSONException
    {
        ...
        Account account = accountDAO.getAccountByAccountName(accountDAO.getAccountName(accountID));
        if(account == null)
        {
            JSONObject jsonObj = new JSONObject();   
            jsonObj.put(JSONPN_STATUS, STATUS_FAILED);
            jsonObj.put(JSONPN_ERROR_MESSAGE, "The account ID is incorrect!");
            logger.error("The security code is incorrect -->" + accountID);
    ...
    public void downloadXLF(HttpServletRequest p_request, HttpServletResponse p_response) throws IOException, JSONException
    {
        ...
        if(account == null)
        {
            ...
            logger.error("Download fail, due the security code is incorrect -->" + accountID);
            p_response.getWriter().write(jsonObj.toString());
            return;
        }        
            ...
            IOException
    {
        ...
        else
        {
            JSONObject jsonObj = new JSONObject();
            jsonObj.put(JSONPN_STATUS, STATUS_FAILED);
            jsonObj.put(JSONPN_ERROR_MESSAGE, "No Account matched by securityCode:" + accountID);
            p_response.getWriter().write(jsonObj.toString());
        }
    }
