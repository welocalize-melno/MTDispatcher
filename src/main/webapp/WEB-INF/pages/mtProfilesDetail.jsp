...
$(document).ready(function() {
	emptyDomainProfileCount();
	fnGetDomains();
	if (GetIEVersion() > 0) { //zero edge version
		$("#idMsMtDomain").css('width','352px');
	}
});
...
function fnInitialData(){
	...
	if ('IPTranslator' == current_engine){
		...
		$("#idAzureSubscriptionKey").val('${mtProfile.subscriptionKey}');
		$("#idMsMtDomain").val(('${mtProfile.domain}') == "0"?"-1":('${mtProfile.domain}'));
...
function checkOptions(formAction) {
    if(formAction == "MS_Translator") {
        ...
        var msTranslatorClientSecret = document.getElementById('idMsMtClientSecret').value;
         var msAzureSubscriptionKey = document.getElementById('idAzureSubscriptionKey').value;
         var flag = (msAzureSubscriptionKey == null || trim(msAzureSubscriptionKey) == "")?true:false;
        var msMtDomain = document.getElementById('idMsMtDomain').value;
        var msMtCategory = document.getElementById('idMsMtCategory').value;       
        var emptyDomainProfileCnt = 0;
        if(msTranslatorUrl == null || trim(msTranslatorUrl) == "") {
            alert("Machine Translation engine url can't be empty or null.");
            return false;
        }
        if((msTranslatorClientID == null || trim(msTranslatorClientID) == "") && flag) {
            //alert("Machine Translation engine client ID can't be empty or null.");
            alert(" Client ID,Client Secret or Azure Subscriptio Key can't be empty or null.");
            return false;
        }
        if((msTranslatorClientSecret == null || trim(msTranslatorClientSecret) == "") && flag) {
            //alert("Machine Translation engine client secret can't be empty or null.");
            alert(" Client ID,Client Secret or Azure Subscriptio Key can't be empty or null.");
            return false;
        }
        if((isAlreadyExists) && (trim(msMtDomain) == "-1" || trim(msMtDomain) == "1")){// && trim(msMtCategory.toLowerCase()) == "general"){// && msAzureSubscriptionKey == ""
        	if(trim(msMtDomain) == "-1"){
        		alert("Please select domain");
        	}
       		if(trim(msMtDomain) == "1"){
    	   		alert("MTProfiles already existed with this domain");
        	}
            return false;
        }    
        if((!isAlreadyExists) && ($.trim(msMtDomain) == "-1")  && (($.trim(msMtCategory) == "")  || $.trim(msMtCategory)!= "general")){
            alert("Machine Translation engine category should be general if domain is empty.");
            return false;
        }
        if($.trim(msMtDomain) == "-1"  && (isAlreadyExists)){
            alert("Please select domain.");
            return false;
        }
        if($.trim(msMtCategory) == ""){
            alert("Please enter category value.");
            return false;
        }
...
//function used to check Untrained domain MTprofile cound
function emptyDomainProfileCount() {
	$.ajax({
		url: "../mtProfiles/emptyDomainProfileCount",
		type: "GET",
		dataType : "json",
		data : { 
			profileId:$('#mtProfileId').val()
		},
		success: function( data ) {
			isAlreadyExists = data.isAlreadyExists;
		},
...
//function used to add new domain if not exists
function fnAddDomain(){
	var domainName = $.trim($('#domainId').val());
	if(domainName == ""){
		$("#domainModalError").html("Please enter domain name");
		return;
	}
	 var $regexname=/^([a-zA-Z ]{3,16})$/;
     if (domainName.length < 3) {
         $('#domainModalError').html("Please enter domain name with mininum 3 characters");
         return;
     }
     if (!domainName.match($regexname)) {
         $('#domainModalError').html("Please enter valid domain name ");
         return;
     }
  	 else{
        $('#domainModalError').html("");
    }
	$('#domainModal #loading').removeClass("hide");
	$.ajax({
        type: 'post',
        data :{
        ...
        dataType: 'json',
        success: function (data) {
        	$('#domainModal #loading').addClass("hide");
        	if(data.isAlreadyExists){
        		$("#domainModalError").html("Domain already existed with this name");
        		return;
        	} 
            if("success" == data.status) {
        		...
        		$('#successMsgModal .modal-body p').html(data.msg);
            } 
            if("timeout" == data.status) {
              location.href = "../home/error.htm";
            } 
            if("error" == data.status) {
                $("#domainModalError").html(data.msg);
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            var msg = "No Response.";
            $("#domainModalError").html(msg);
            $('#domainModal #loading').addClass("hide");
        }
    });
	return;
}
//Function used to open a modal window to add new domain
function openDomainModal(){
	$('#domainModal').on('shown.bs.modal', function(){	
		$('#domainModal .modal-body #domainId').val("");
		$('#domainModal .modal-body #domainModalError').html("");
		$('#domainModal #loading').addClass("hide");
	}).modal({backdrop: 'static', keyboard: false, show:true});
}
//Function used to get all existing domain
	...
	$.ajax({
		...
		success: function( data ) {
			...
			if(domains != null && domains.length > 0){
				$("#idMsMtDomain").append("<option value='-1'></option>");
				for(i=0; i<domains.length; i++){
					$("#idMsMtDomain").append("<option value='"+domains[i].id+"'>"+domains[i].domainName+"</option>");
				}
			}
			fnInitialData();
		},
...
<DIV>
		...
		<form action="" method="POST" id="MTOptionsForm" name="MTOptionsForm">
			...
			<input type="hidden" value="${mtProfile.id}" name="mtProfileId" id="mtProfileId"> 
			<div id="baseInfo">
				<table cellspacing="2" cellpadding="2" border="0" class="standardText">
					...
					<div style="display: block;" id="msMtDiv">
							...
							<table width="82%" cellspacing="2" cellpadding="2" border="0" class="standardText">
								...
								<tr>
									<td colspan="3"><b>It is required to input value(s) for either 'Client ID','Client Secret' or 'Azure Subscription Key'.</b></td>
								</tr>
								<tr>
									<td align="LEFT">Client ID:</td>
									<td><input type="text" size="60" maxlength="100" value=" " name="ms_mt_client_id" id="idMsMtClientid" class="standardText"> 
									</td>
								</tr>
								<tr>
								...
								<tr>
									<td align="LEFT">Azure Subscription Key:</td>
									<td><input type="text" size="60" maxlength="100" value="null " name="azure_subscription_key" id="idAzureSubscriptionKey" class="standardText">&nbsp;<a target="_blank" href="http://go.microsoft.com/?linkid=9782667">(Get one)</a>
									</td>
								</tr>
								<tr>
									...
									<td>
										<select id="idMsMtDomain" class="standardText" name="ms_mt_domain" onChange="emptyDomainProfileCount()">
										</select>&nbsp;<a onClick="openDomainModal()">Add new domain</a>
									</td>
								</tr>
								<tr>
									<td align="LEFT">Category:
									</td>
									<td colspan="2"><input type="text" size="60" maxlength="60" value="general" name="ms_mt_category" id="idMsMtCategory" class="standardText"> 
									</td>
								</tr>
							</tbody></table>
							</p><p>
	...
	<%@ include file="/WEB-INF/pages/domain.jspIncl" %>
</DIV>
</body>
</html>